...
const VerVehiculos = () => {
    ...
    const handleSubmitVehiculoFiltro = async(e) => {
        e.preventDefault()
        mostrarMensajeConfirmacion("Esta seguro de modificar la cuota con el filtro ",selectedBrand,selectedCategory)
    }

    function mostrarMensajeConfirmacion(mensaje, marca, categoria) {
        Swal.fire({
            icon: 'warning', // Puedes usar 'success', 'error', 'warning', 'info', etc.
            title: mensaje + " " + marca + " " + categoria,
            showCancelButton: true, // Muestra el botón de Cancelar
            confirmButtonText: 'Aceptar', // Etiqueta del botón de Aceptar
            ...
            if (result.isConfirmed) {
                if (marca != '' && categoria != '') {
                    ...
                    try {
                        const response = fetch('http://localhost:5000/modificar_cuota_vehiculo_ambos', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                            },
                    ...
                    try {
                        const response = fetch('http://localhost:5000/modificar_cuota_vehiculo_marca', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                            },
                    ...
                    try {
                        const response = fetch('http://localhost:5000/modificar_cuota_vehiculo_categoria', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                            },
                    ...
                    console.log("No se escogio ningun filtro")
                }
            } else if (result.dismiss === Swal.DismissReason.cancel) {
                // Aquí puedes colocar el código a ejecutar cuando se hace clic en "Cancelar"
            }
        });
    }
            ...
            <div className="container mt-5">
                ...
                <div className="row mb-3">
                    ...
                    <div className='col-md-3 text-center'>
                        <h6>Modificar Cuota Filtra</h6>
                        <Button className="btn btn-secondary" onClick={handleSubmitVehiculoFiltro}>
